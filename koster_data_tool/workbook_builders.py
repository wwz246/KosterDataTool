...
from .excel_writer import blank_col_after, write_block
    ...
    for b in sorted(scan_result.batteries, key=lambda x: x.name):
        ...
        for g in b.gcd_files:
            ...
            for k in range(1, int(b.gcd_max_cycle or 0) + 1):
                ...
                if rep is None:
                    continue
                ce = (100.0 * rep.delta_q_dis / rep.delta_q_chg) if rep.delta_q_chg and rep.delta_q_dis else None
                ...
                delta_v = max(1e-12, float(bp.get("v_end", 0.0)) - float(bp.get("v_start", 0.0)))
                kf = float(bp.get("k", 1.0))
                csp_chg_eff = ((rep.delta_q_eff_chg * 3.6) / (delta_v * m_active_g) * kf) if rep.delta_q_eff_chg is not None else None
                csp_dis_eff = ((rep.delta_q_eff_dis * 3.6) / (delta_v * m_active_g) * kf) if rep.delta_q_eff_dis is not None else None
...
def build_battery_workbook(scan_result, params, logger, run_report_path) -> Workbook:
    ...
    for b in sorted(scan_result.batteries, key=lambda x: x.name):
        ...
        write_block(ws, col, er + 1, retention)
